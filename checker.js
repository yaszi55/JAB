  ...
  for (let yellow of yellowSquares) {
    ...
    for (let orange of orangeSquares) {
      ...
      if (overlapArea / yellowArea >= 0.7) {
        const row = Math.floor(oRect.top / (squareSize + gap));
        const col = Math.floor(oRect.left / (squareSize + gap));
...
function checkAttachedYellowNeighbors({
  ...
  for (let red of allRedSquares) {
    const row = Math.floor(red.offsetTop / (squareSize + gap));
    const col = Math.floor(red.offsetLeft / (squareSize + gap));
    grid[row][col] = 1;
